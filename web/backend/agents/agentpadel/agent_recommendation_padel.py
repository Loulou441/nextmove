...
from agents.agentmanager.schemas import RecommandationsCoach
from agents.agentmanager.exceptions import EmptyResponseError, InvalidResponseError
from agents.agentmanager.rag import get_knowledge_base, enrich_match_data_with_drills
from config import MODEL_NAME_PADEL, GROQ_TEMPERATURE

logger = logging.getLogger("nextmove.agents.padel")

KNOWLEDGE_PATH = Path(__file__).resolve().parent / "knowledge_padel.json"
...
class PadelCoachAI(Agent):
    def __init__(self, context, user_prompt):
        ...
        self.knowledge_base = get_knowledge_base("padel", KNOWLEDGE_PATH)
    ...
    def generate_recommendations(self, match_data) -> RecommandationsCoach:
        ...
        try:
            ...
            logger.exception("Échec de génération des recommandations padel.")
            raise
...
def main():
    ...
    with open(base_dir / "context_padel.txt", "r", encoding="utf-8") as f:
        context = f.read()

    with open(base_dir / "user_prompt_padel.txt", "r", encoding="utf-8") as f:
        prompt = f.read()

    coach = PadelCoachAI(context, prompt)
