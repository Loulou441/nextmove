...
Prérequis : Python 3.10+ (le code utilise la syntaxe `str | None`). Les
dépendances (`fastapi`, `uvicorn`, `email-validator`, ...) sont dans le
`requirements.txt` partagé avec l'app Streamlit — pas de venv séparé.
...
Depuis `streamlit/`, créer les tables puis démarrer :
