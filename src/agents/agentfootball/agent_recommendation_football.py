...
class FootballCoachAI(Agent):
    ...
    def generate_recommendations(self, match_data) -> RecommandationsCoach:
        ...
        validées structurellement (RecommandationsCoach).

        Résilience : les erreurs réseau/serveur Groq et les réponses mal
        formées sont automatiquement retentées (voir Agent.call_and_validate).
        Si toutes les tentatives échouent, l'exception est journalisée puis
        propagée pour que l'appelant décide de la conduite à tenir (message
        d'erreur utilisateur, alerte monitoring...).
        """
        messages = [
